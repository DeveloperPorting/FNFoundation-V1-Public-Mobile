...
class Character extends FlxSprite
{
	...
	public static function preload(character:String)
	{
	...
	public function load(?character:String = 'bf')
	{
		#if (haxe >= "4.0.0")
		animOffsets = new Map();
		#else
		...
		if (!isAnimateAtlas)
		{
			switch (animation.curAnim.name.substr(4).toLowerCase())
			{
				...
				case 'right' | 'right-alt':
					return displace.set(baseCameraDisplacement, 0);
				default:
					return displace.set(0, 0);
			}
		}
		else
		{
			// switch (atlas.substr(4).toLowerCase())
			// {
			// 	case 'up' | 'up-alt':
			// 		return displace.set(0, -baseCameraDisplacement);
			// 	case 'down' | 'down-alt':
	...
	public function playGhostAnim(ghostID = 0, AnimName:String, Force:Bool = false, Reversed:Bool = false, Frame:Int = 0)
	{
		...
		var moveDirections:Array<Float> = [x + (directionMap.get(direction)[0]), y + (directionMap.get(direction)[1])];
	...
	// This shit is stolen straight from psych engine
	// Atlas support
	@:allow(meta.states.editors.CharacterEditorState)
